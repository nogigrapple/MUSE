...
class MUSE(nn.Module):
    def __init__(self, n_users, n_items, feat_embed_dim, nonzero_idx, nonzero_idx_img, nonzero_idx_txt, has_norm, image_feats, text_feats, train_items, n_layers, alpha, beta, gamma, delta):
        ...
        self.iceuce = ICEnUCE(self.n_users, self.n_items, self.n_layers, has_norm, self.feat_embed_dim, image_feats, text_feats, train_items, self.delta, self.adj) 
    ...
    def forward(self, users, neg_items, eval=False): 
        if eval:                  
            user_ice, item_ice, image_query, text_query, comp_rel_v, comp_rel_t, comp_rel_v_neg, comp_rel_t_neg, v_rel_mlp, t_rel_mlp, image_feats, text_feats = self.iceuce(self.edge_index, self.edge_weight, users, neg_items, eval=True)            
        ...
        user_ice, item_ice, image_query, text_query, comp_rel_v, comp_rel_t, comp_rel_v_neg, comp_rel_t_neg, v_rel_mlp, t_rel_mlp, image_feats, text_feats = self.iceuce(self.edge_index, self.edge_weight, users, neg_items, eval=False)                           
    ...
    def bpr_loss(self, user_ice, item_ice, user_mce, item_mce, image_query, text_query, comp_rel_v, comp_rel_t, comp_rel_v_neg, comp_rel_t_neg, users, pos_items, neg_items, pos_pairs):
        ...
        # predictor
        pos_uce_score = torch.sum(torch.mul(user_standpoint, current_cmp_rel), dim=1)
        neg_uce_score = torch.sum(torch.mul(user_standpoint, current_cmp_rel_neg), dim=1) 
        pos_scores = (1 - self.gamma) * torch.sum(torch.mul(current_ice_user_emb, ice_pos_item_emb), dim=1) + self.gamma * torch.sum(torch.mul(pos_target_user, ice_pos_item_emb), dim=1) + self.alpha * torch.sum(torch.mul(current_mce_user_emb, mce_pos_item_emb), dim=1)+self.beta*pos_uce_score
        neg_scores = (1 - self.gamma) * torch.sum(torch.mul(current_ice_user_emb, ice_neg_item_emb), dim=1) + self.gamma * torch.sum(torch.mul(neg_target_user, ice_neg_item_emb), dim=1) + self.alpha * torch.sum(torch.mul(current_mce_user_emb, mce_neg_item_emb), dim=1)+self.beta*neg_uce_score
        ...
        uce_regularizer = 1./2*(user_standpoint**2).sum() + 1./2*(current_cmp_rel**2).sum() + 1./2*(current_cmp_rel_neg**2).sum()  
        ice_emb_loss = ice_regularizer / ice_pos_item_emb.size(0)
        mce_emb_loss = mce_regularizer / mce_pos_item_emb.size(0)
        uce_emb_loss = uce_regularizer / current_cmp_rel.size(0) 
        emb_loss = ice_emb_loss+mce_emb_loss+uce_emb_loss
